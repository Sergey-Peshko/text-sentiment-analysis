...
def train(epoch_amount, data, session, x, y_true, batch_size, accuracy, cost, optimizer, report_file_name):
    ...
    train_data_accuracy_manager = DataSetAccuracyManager(data.train, session, x, y_true, 800)
    valid_data_accuracy_manager = DataSetAccuracyManager(data.valid, session, x, y_true, 800)
    ...
    while passed_epoch < epoch_amount:
        ...
        feed_dict_tr = {x: x_batch,
                        y_true: np.transpose([y_true_batch])}
        ...
        if dataset_epoch_manager.epochs_done == (passed_epoch + 1):
            ...
            val_loss = valid_data_accuracy_manager.calc_loss(cost)[0]
